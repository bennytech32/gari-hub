...
export default function Home() {
  ...
  // SERVICE & INSPECTION BOOKING STATES
  const [showServiceModal, setShowServiceModal] = useState(false);
  const [serviceData, setServiceData] = useState({ name: '', phone: '', carModel: '', serviceType: 'Computer Diagnostics (OBD2)', date: '' });
  ...
  const submitServiceBooking = async (e: React.FormEvent) => {
    ...
    try {
      const messagePayload = `SERVICE BOOKING\nJina: ${serviceData.name}\nGari Lake: ${serviceData.carModel}\nHuduma Anayotaka: ${serviceData.serviceType}\nSiku: ${serviceData.date}`;
      await supabase.from('inquiries').insert([{ contact_phone: serviceData.phone, customer_message: messagePayload }]);
      alert(lang === 'sw' ? "Ombi lako limepokelewa. Fundi wetu atakupigia!" : "Booking received! Our mechanic will contact you.");
      setShowServiceModal(false); setServiceData({ name: '', phone: '', carModel: '', serviceType: 'Computer Diagnostics (OBD2)', date: '' });
  ...
  const t = {
    sw: {
      home: "Mwanzo", inventory: "Magari Yetu", sparesMenu: "Vipuri & Vifaa", calcMenu: "Kikokotozi", contact: "Wasiliana Nasi", servicesMenu: "Service & Kaguzi",
      ...
      modServTitle: "Book Service / Inspection", modServDesc: "Weka miadi na mafundi wetu kwa ajili ya Diagnostics, Full Service, au kukagua gari kabla hujanunua.", modServType: "Aina ya Huduma", modServCar: "Gari Lako / Linalokaguliwa", modServDate: "Tarehe", modServBtn: "Weka Miadi Sasa 🚀",
      ...
      card1Title: "Kaguzi (Inspection)", card1Desc: "Usipate hasara kununua gari bovu. Mafundi wetu watakagua injini na mfumo wa umeme kwa kutumia OBD2 Scanners.", card1Btn: "Weka Miadi Sasa ➔",
    ...
    en: {
      home: "Home", inventory: "Our Inventory", sparesMenu: "Parts & Tools", calcMenu: "Calculator", contact: "Contact Us", servicesMenu: "Services & Inspection",
      ...
      modServTitle: "Book Service / Inspection", modServDesc: "Book an appointment with our expert mechanics for Diagnostics, Full Service, or Pre-purchase inspection.", modServType: "Service Type", modServCar: "Your Car Make & Model", modServDate: "Preferred Date", modServBtn: "Book Appointment Now 🚀",
      ...
      card1Title: "Pre-Purchase Inspection", card1Desc: "Don't risk buying a bad car. Our mechanics inspect engine and electronics using advanced OBD2 Scanners.", card1Btn: "Book Appointment ➔",
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-0 font-sans text-gray-800 relative">
              ...
              <form onSubmit={submitServiceBooking} className="space-y-4">
                <div>
                  ...
                  <select value={serviceData.serviceType} onChange={(e) => setServiceData({...serviceData, serviceType: e.target.value})} className="w-full border-2 border-gray-100 p-3.5 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-gray-50 text-[16px] font-bold text-emerald-800 outline-none">
                    ...
                    <option>{lang === 'sw' ? 'General Service (Oil & Plugs)' : 'General Service (Oil & Plugs)'}</option>
                    <option>{lang === 'sw' ? 'Ushauri wa Kiufundi' : 'Technical Consultation'}</option>
                  </select>
                </div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1.5 uppercase tracking-wider">{currentT.modServCar}</label><input type="text" required value={serviceData.carModel} onChange={(e) => setServiceData({...serviceData, carModel: e.target.value})} placeholder="E.g. Toyota Crown 2012" className="w-full border-2 border-gray-100 p-3.5 rounded-xl outline-none focus:border-emerald-500 bg-gray-50 text-[16px]" /></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-[10px] font-bold text-gray-700 mb-1.5 uppercase tracking-wider">{currentT.modServDate}</label><input type="date" required value={serviceData.date} onChange={(e) => setServiceData({...serviceData, date: e.target.value})} className="w-full border-2 border-gray-100 p-3.5 rounded-xl outline-none focus:border-emerald-500 bg-gray-50 text-[16px]" /></div>
                </div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1.5 uppercase tracking-wider">{currentT.modPhone}</label><input type="text" required value={serviceData.phone} onChange={(e) => setServiceData({...serviceData, phone: e.target.value})} placeholder="07XX XXX XXX" className="w-full border-2 border-gray-100 p-3.5 rounded-xl outline-none focus:border-emerald-500 bg-gray-50 text-[16px] font-bold" /></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden lg:flex space-x-6 items-center">
              ...
              <Link href="/calculator" className="text-slate-600 hover:text-blue-600 px-1 py-2 font-semibold transition-colors text-sm">{currentT.calcMenu}</Link>
              <button onClick={() => setShowServiceModal(true)} className="text-slate-600 hover:text-blue-600 px-1 py-2 font-semibold transition-colors text-sm">{currentT.servicesMenu}</button>
            </div>
      ...
      {/* HERO SECTION - PREMIUM DARK MODE NA UREFU ULIOREKEBISHWA */}
            ...
            <main className="mt-2 mx-auto max-w-7xl sm:mt-6 md:mt-8 lg:mt-8">
              <div className="text-center lg:text-left">
                ...
                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row sm:justify-center lg:justify-start gap-4">
                  ...
                  <div className="rounded-xl shadow-lg w-full sm:w-auto">
                    <Link href="/calculator" className="w-full flex items-center justify-center px-6 sm:px-8 py-3.5 sm:py-4 border border-slate-600 text-sm sm:text-base font-bold rounded-xl text-white bg-slate-800/60 backdrop-blur-md hover:bg-slate-700 hover:border-slate-500 transition-all">
                      {currentT.calcMenu}
                    </Link>
                  </div>
                </div>
      ...
      {/* ADVANCED QUICK SEARCH - SLEEK DESIGN */}
      ...
      <div id="magari" className="max-w-[100%] md:max-w-[95%] mx-auto px-4 sm:px-6 lg:px-8 mt-12 sm:mt-16 mb-16">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          {/* SIDEBAR - SLEEK & COMPACT (w-full kwenye simu, w-1/5 kwenye PC) */}
          <div className="w-full lg:w-1/5 order-1 lg:order-1">
            <div className="bg-white rounded-2xl shadow-sm p-4 lg:p-6 border border-gray-100 lg:sticky lg:top-28">
              ...
              {/* Kwenye Simu inakaa kama Grid (Safu mbili mbili), PC inakaa chini chini */}
      ...
      {/* SERVICES HUB - MODERN CARDS */}
      <div id="services-hub" className="bg-white py-16 sm:py-20 border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-16">
            <span className="bg-blue-50 text-blue-600 border border-blue-200/50 text-[9px] sm:text-[10px] font-black px-3 sm:px-4 py-1.5 rounded-full uppercase tracking-widest mb-3 sm:mb-4 inline-block">{currentT.srvSubtitle}</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight">{currentT.srvTitle}</h2>
            <p className="mt-3 sm:mt-4 text-slate-500 max-w-2xl mx-auto text-sm sm:text-base px-4">{currentT.srvDesc}</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            
            <div onClick={() => setShowServiceModal(true)} className="bg-slate-50 rounded-3xl p-6 sm:p-8 border border-gray-100 hover:bg-white hover:shadow-2xl hover:border-emerald-100 transition-all duration-500 group cursor-pointer flex flex-col h-full transform sm:hover:-translate-y-2">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-emerald-100 text-emerald-600 rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-5 sm:mb-6 shadow-sm group-hover:scale-110 transition-transform duration-500">🛠️</div>
              <h3 className="font-black text-base sm:text-lg text-slate-900 mb-2 sm:mb-3">{currentT.card1Title}</h3>
              <p className="text-slate-500 text-xs sm:text-sm mb-6 sm:mb-8 flex-grow leading-relaxed">{currentT.card1Desc}</p>
              <div className="mt-auto"><span className="text-emerald-600 font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all">Book Now <span>➔</span></span></div>
            </div>

            <Link href="/spares" className="bg-slate-50 rounded-3xl p-6 sm:p-8 border border-gray-100 hover:bg-white hover:shadow-2xl hover:border-blue-100 transition-all duration-500 group cursor-pointer flex flex-col h-full transform sm:hover:-translate-y-2">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-5 sm:mb-6 shadow-sm group-hover:scale-110 transition-transform duration-500">⚙️</div>
              <h3 className="font-black text-base sm:text-lg text-slate-900 mb-2 sm:mb-3">{currentT.card2Title}</h3>
              <p className="text-slate-500 text-xs sm:text-sm mb-6 sm:mb-8 flex-grow leading-relaxed">{currentT.card2Desc}</p>
              <div className="mt-auto"><span className="text-blue-600 font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all">Visit Shop <span>➔</span></span></div>
            </Link>

            <Link href="#reviews" className="bg-slate-50 rounded-3xl p-6 sm:p-8 border border-gray-100 hover:bg-white hover:shadow-2xl hover:border-orange-100 transition-all duration-500 group cursor-pointer flex flex-col h-full transform sm:hover:-translate-y-2">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-orange-100 text-orange-600 rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-5 sm:mb-6 shadow-sm group-hover:scale-110 transition-transform duration-500">📝</div>
              <h3 className="font-black text-base sm:text-lg text-slate-900 mb-2 sm:mb-3">{currentT.card3Title}</h3>
              <p className="text-slate-500 text-xs sm:text-sm mb-6 sm:mb-8 flex-grow leading-relaxed">{currentT.card3Desc}</p>
              <div className="mt-auto"><span className="text-orange-600 font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all">Read Articles <span>➔</span></span></div>
            </Link>

            <Link href="/calculator" className="bg-slate-50 rounded-3xl p-6 sm:p-8 border border-gray-100 hover:bg-white hover:shadow-2xl hover:border-purple-100 transition-all duration-500 group cursor-pointer flex flex-col h-full transform sm:hover:-translate-y-2">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-purple-100 text-purple-600 rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-5 sm:mb-6 shadow-sm group-hover:scale-110 transition-transform duration-500">🧮</div>
              <h3 className="font-black text-base sm:text-lg text-slate-900 mb-2 sm:mb-3">{currentT.card4Title}</h3>
              <p className="text-slate-500 text-xs sm:text-sm mb-6 sm:mb-8 flex-grow leading-relaxed">{currentT.card4Desc}</p>
              <div className="mt-auto"><span className="text-purple-600 font-black text-[10px] sm:text-xs uppercase tracking-widest flex items-center gap-2 group-hover:gap-3 transition-all">Calculate <span>➔</span></span></div>
            </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-12 mb-12 sm:mb-16 text-center md:text-left">
            ...
            <div>
              ...
              <ul className="space-y-3 text-xs sm:text-sm font-medium">
                ...
                <li><Link href="/calculator" className="hover:text-blue-400 transition-colors">{currentT.calcMenu}</Link></li>
                <li><button onClick={() => setShowServiceModal(true)} className="hover:text-blue-400 transition-colors">{currentT.servicesMenu}</button></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-3 text-xs sm:text-sm font-medium">
                ...
                <li><Link href="/" className="hover:text-blue-400 transition-colors">Customs Clearing</Link></li>
              </ul>
            </div>
